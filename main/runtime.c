...
void w4_runtimeBlit(const uint8_t* sprite, int x, int y, int width, int height,
                    int flags) {
...
void w4_runtimeBlitSub(const uint8_t* sprite, int x, int y, int width,
                       int height, int srcX, int srcY, int stride, int flags) {
  // printf("blitSub: %p, %d, %d, %d, %d, %d, %d, %d, %d\n", sprite, x, y,
  // width, height, srcX, srcY, stride, flags);

  bool bpp2 = (flags & 1);
  bool flipX = (flags & 2);
  bool flipY = (flags & 4);
  bool rotate = (flags & 8);
  w4_framebufferBlit(sprite, x, y, width, height, srcX, srcY, stride, bpp2,
                     flipX, flipY, rotate);
}
...
void w4_runtimeTrace(const uint8_t* str) { puts(str); }
    ...
    if (*str == '%') {
      ...
      case 's':
        ...
        if (strPtr > 0 && strPtr < sizeof(*memory)) {
          printf("%.*s", (int)(sizeof(*memory) - strPtr),
                 (char*)memory + strPtr);
        } else {
...
int w4_runtimeSerializeSize() { return sizeof(SerializedState); }
